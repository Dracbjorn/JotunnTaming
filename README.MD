...
6. Fork and clone this repository using git. That should create a new folder `JotunnTaming`. You can also [use the template function of github](https://github.com/Valheim-Modding/JotunnTaming/generate) to create a new, clean repo out of it and clone that.
7. Create a new environment file `Environment.props` in the projects base path `<JotunnTaming>`. Paste this snippet and change the path according to your local Valheim installation.
```xml
...
9. Open the Solution file `<JotunnTaming>\JotunnTaming.sln`. Right-click on the project or solution in the Solution Explorer and select `Manage NuGet packages...`. It should prompt you a message at the top that some NuGet-Packages are missing. Click "Restore" and restart Visual Studio when finished.
...
* `<JotunnTaming>\libraries\Debug\mono-2.0-bdwgc.dll` is copied to `<ValheimDir>\MonoBleedingEdge\EmbedRuntime` replacing the original file (a backup is created before).

### Building Release

* A compressed file with the binaries is created in `<JotunnTaming>\Packages`ready for upload to ThunderStore. Dont forget to include your information in the manifest.json and to change the project's readme file.

## Developing Assets with Unity

New Assets can be created with Unity and imported into Valheim using the mod. A Unity project is included in this repository under `<JotunnTaming>\JotunnModUnity`.
...
4. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `<JotunnTaming>\JotunnModUnity\Assets\Assemblies`. **Do this directly in the filesystem - don't open Unity first or import the dlls directly in Unity**.
...
4. Load `JotunnTaming.dll` from `<ValheimDir>\BepInEx\plugins` into dnSpy.
5. Copy `<JotunnTaming>\libraries\Debug\mono-2.0-bdwgc.dll` into `<ValheimDir>\MonoBleedingEdge\EmbedRuntime` and overwrite the existing file.
...
3. Build the project with target `Debug`. The publish.ps1 PowerShell script from this repo...
   ...
   * automatically generates a JotunnTaming.dll.mdb file, which is needed for Unity/mono debugging. It should be in \<ValheimDir>\BepInEx\plugins, too.
...
1. Copy all `assembly_*.dll` from `<ValheimDir>\valheim_Data\Managed` into `<JotunnTaming>\JotunnModUnity\Assets\Assemblies`. <br />
